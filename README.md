...
- 基隆垃圾車來了公開車輛/路線 API：`garbage-truck`（基隆市；公開即時車輛 GPS + 路線/站點，無近期車輛時不推播）
...
- 清運e點通 ASP.NET / 車輛 JSON 即時查詢：`garbage-truck`（南投、彰化、臺東、澎湖部分鄉鎮；無執勤車輛時回環境部路線 fallback）
...
- 新竹縣政府路邊停車中心停車場查詢：`parking`（新竹縣；解析官方停車場、地址、收費時間與費率，未提供即時剩餘車位 API）
- 苗栗縣政府停車服務資訊收費路段：`parking`（苗栗縣；解析官方收費路段、起訖、收費時間與費率，未提供即時剩餘車位 API）
- 南投縣路邊智慧車格與停車場清單：`parking`（南投縣；官方智慧車格 POST API + 停車場清單）
...
- 臺東縣停車資訊網停車場車位表：`parking`（臺東縣；解析官方停車場車位數，未提供即時剩餘車位 API）
...
- PTX/MOTC 捷運營運警示：`transit`（北捷、桃捷、中捷、高捷；即時營運狀態，業者官網作為補底）
...
- PTX/MOTC 高雄捷運營運警示：`transit`（高雄市；即時營運狀態，重要公告作為補底）
